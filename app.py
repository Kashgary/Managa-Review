...
def create_app(test_config=None):
    ...
    def add_manga(jwt):
        ...
        if not ('title' in body and 'author' in body and 'genre'
                in body and 'rating' in body):
            abort(422)
    ...
    def get_reviews(manga_id):
        ...
        manga = Manga.query.with_entities(Manga.title).filter(
            Manga.id == manga_id).all()
    ...
    def add_review(jwt, manga_id):
        ...
        if not manga:
            abort(404)
        try:
            review = Review(title=body.get('title'),
                            name=body.get('name'),
            ...
            return jsonify({'success': True, 'created': review.title})
        except BaseException:
    ...
    def delete_review(jwt, id):
        manga_id = \
            Review.query.with_entities(Review.manga_id).filter(
                Review.id == id).all()
        if not manga_id:
            abort(404)
    ...
    def handle_auth_error(ex):
        return (jsonify({'success': False, 'error': ex.status_code,
                'message': ex.error}), 401)
